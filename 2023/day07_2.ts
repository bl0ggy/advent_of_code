...
const listOfCards = ['J', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'Q', 'K', 'A'] as const;
...
function getKind(a: Hand) : [number, Hand] {
    ...
    if(threeOfAKind.length === 1 && twoOfAKind.length === 1) { return [5, updatedHand]; }
...
function compareHands(a: HandSet, b: HandSet) : number {
    const kindA: [number, Hand] = getKind(a.hand);
    const kindB: [number, Hand] = getKind(b.hand);
    if(kindA[0] === kindB[0]) {
